...
function normalizeRow(cells: string[], map: Partial<Record<keyof PurchaseMasterInput, number>>): PurchaseMasterInput | null {
  const row: PurchaseMasterInput = {
    fy: getCell(cells, map.fy),
    gst_no: getCell(cells, map.gst_no).toUpperCase(),
    status: getCell(cells, map.status).toUpperCase() === "INACTIVE" ? "INACTIVE" : "ACTIVE",
